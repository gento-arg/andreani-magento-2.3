        ...
        if ($this->andreaniHelper->getWebserviceMethod() == 'soap') {
            ...
            try {
                foreach ($sucursales as $item_sucursal) {
                    try {
                        $direccion_full = explode(' , ', $item_sucursal->Direccion);

                        if ($needFilter) {
                            if ((!is_null($region) && $region != trim($direccion_full[3])) || (!is_null($location) && $location != trim($direccion_full[2]))) {
                                continue;
                            }
                        ...
                        $sucursal->setStreet(trim($direccion_full[0]));
                        $sucursal->setPostcode(trim($direccion_full[1]));
                        $sucursal->setLocation(trim($direccion_full[2]));
                        $sucursal->setRegion(trim($direccion_full[3]));
    ...
    public function fetchSucursalesCP($cp)
    {
        ...
        if ($this->andreaniHelper->getWebserviceMethod() == 'soap') {
            ...
            foreach ($sucursales as $item_sucursal) {
                try {
                    if ($needFilter) {
                        if ((!is_null($cp) && strtoupper($cp) != strtoupper($item_sucursal["direccion"]['codigoPostal']))) {
                            continue;
                        }
                    }
